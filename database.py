...
from src.strings import get_now_date
...
def add_user(user_id):
    ...
    with connect_db() as db:
        ...
        if len(user) == 0:
            cur.execute("INSERT INTO UserList (user_id) VALUES (?)", [user_id])
            return True
        else:
            return False
...
def add_buy(purchase_data):
    ...
    with connect_db() as db:
        cur = db.cursor()
        cur.execute("INSERT INTO Sales (user_id, item_name, amount, count, date) VALUES (?, ?, ?, ?, ?)",
                    (purchase_data['user_id'], purchase_data['item_name'], purchase_data['amount'],
                     purchase_data['count'], purchase_data['date']))

        return cur.execute("SELECT * FROM Sales WHERE user_id = ? AND item_name = ? AND amount = ? AND count = ? AND "
                           "date = ?",
                           (purchase_data['user_id'], purchase_data['item_name'], purchase_data['amount'],
                            purchase_data['count'], purchase_data['date'])).fetchone()[0]
...
def add_categories(category_list):
    ...
    with connect_db() as db:
        ...
        for i in range(len(category_list)):
            cur.execute("INSERT INTO Category (name) VALUES (?)", [category_list[i]])
    ...
    with connect_db() as db:
        ...
        if if_delete:
            ...
            for i in range(len(item_list)):
                cur.execute("DELETE FROM Items WHERE id = ?", [item_list[i][0]])
...
def edit_name(item_id, name):
    ...
    with connect_db() as db:
        cur = db.cursor()
        cur.execute("UPDATE ItemList SET name = ? WHERE id = ?", (name, item_id))


def edit_desc(item_id, desc):
    """
    ...
    with connect_db() as db:
        cur = db.cursor()
        cur.execute("UPDATE ItemList SET desc = ? WHERE id = ?", (desc, item_id))


def edit_pic(item_id, pic):
    """
    Редактирование названия товара
    ...
    with connect_db() as db:
        cur = db.cursor()
        cur.execute("UPDATE ItemList SET pic = ? WHERE id = ?", (pic, item_id))


def edit_price(item_id, price):
    """
    Редактирование названия товара
    ...
    with connect_db() as db:
        cur = db.cursor()
        cur.execute("UPDATE ItemList SET price = ? WHERE id = ?", (price, item_id))
...
def register_support(data):
    ...
    with connect_db() as db:
        cur = db.cursor()
        cur.execute("INSERT INTO Support (user_id, username, message, type, state, answer, date) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (data['user_id'], data['username'], data['message'], data['type'], 0, "", get_now_date()))
